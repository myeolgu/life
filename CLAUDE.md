...
- **인테리어** — 부개주공1단지 아파트 107동 1001호(인천광역시 부평구 부개동 — 부천시 아님, 처음에 잘못 가정했던 적 있음. 25평/전용 약 59㎡) 인테리어 준비 (시공업체 비교·견적서, 요구사항 대조, 매물정보/시공범위/공사순서/계약견적 체크리스트)
- **디딤돌 대출/계약** — 혼인신고, 신혼부부 디딤돌대출 신청 진행 상황과 자금 계획, 필요서류
...
- **시드를 고쳐도 DB의 기존 행은 자동으로 갱신되지 않는다 — 세 테이블 모두 해당.** 훅(`useChecklist`/`useContentItems`/`useCalendarEvents`)은 해당 도메인/섹션 행이 하나도 없을 때만 시드를 넣으므로, 이미 방문된 적이 있으면 `data.js` 수정만으로는 화면이 바뀌지 않는다. `data.js`를 고칠 때마다 Management API로 DB도 같이 맞춘다:
  ...
  - 반영한 뒤에는 `select`로 다시 읽어서 시드와 일치하는지 확인한다.

## 로컬 빌드
2026-09-23 기준 작업 폴더는 ASCII 경로 `C:\life\life`이고 `npm run build`가 로컬에서 정상 동작한다. (예전 한글 경로 `C:\workspace\인테리어`에서는 esbuild가 exit code 127로 실패했었다 — 작업 폴더를 다시 한글 경로로 옮기지 말 것.)
...
- **화면에 올리는 사실 정보(정책 조건, 서류 발급처/유효기간, 금리·한도 등)에는 항상 출처를 남긴다** (2026-09-06, 사용자가 명시적으로 요청 — "모든 정보에는 근거가 있어야해 출처나"). 웹서치로 확인한 내용만 반영하고, 확실한 공식 출처를 못 찾았으면 "확인 안 됨"이라고 솔직히 표시하지 추측으로 채우지 않는다. UI에 넣을 때는 텍스트에 매체명만 적지 말고 실제로 검색해서 찾은 URL로 링크를 건다 (`후기 & 꿀팁` 탭의 `tips` 카드처럼 `title`/`summary`/`source`/`url` 필드를 갖춰서 `content_items`에 저장하는 패턴 참고, `src/domains/loan/LoanPage.jsx`의 `Tips` 컴포넌트). URL은 실제 검색 결과에서 나온 것만 쓰고 절대 지어내지 않는다.
- **시공업체 후보는 실내건축공사업 등록 여부를 반드시 확인하고 카드에 남긴다.** 조회처는 kiscon.net(건설산업지식정보시스템)이고, 조회 결과와 날짜를 `note`에 적는다 (예: "실내건축공사업 등록 확인됨 (kiscon.net 조회, 2026-09-14)"). 확인 전이면 `verified: false`. 공사예정금액 1,500만원 이상인 실내건축공사는 등록업체만 할 수 있으므로(건설산업기본법 시행령 제8조), 현재 견적 규모(3천만원대)에서는 **등록 없는 업체와는 계약하지 않는 것이 원칙**이다 — 사용자가 비교용으로 상담만 받는 건 괜찮고, 그 경우에도 카드에 이 원칙을 명시한다.
