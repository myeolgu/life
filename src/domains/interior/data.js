...
export const contractors = [
  {
    ...
    verified: true,
    // 2026-09-20: 2026-09-19자 서면 견적서(9페이지)를 항목별로 읽어 옮긴 것. 페이지 소계·재료비/노무비
    // 합계가 표지 금액과 전부 일치하는 걸 계산으로 확인함. VAT 포함 여부는 사용자가 업체에 확인.
    quote: {
      kind: "written",
      date: "2026-09-19",
      total: 34140167,
      vat: "포함 (사용자 확인, 2026-09-20)",
      breakdown: [
        { label: "철거공사", amount: 2500000 },
        { label: "도배공사", amount: 2740000 },
        { label: "바닥재 (LX 3.2T, 21평)", amount: 1956150 },
        { label: "필름공사", amount: 1210000 },
        { label: "제작가구 (사제품)", amount: 5747550 },
        { label: "조명·전기", amount: 3330500 },
        { label: "타일공사", amount: 2955700 },
        { label: "거실욕실", amount: 4085000 },
        { label: "목공사", amount: 6672000 },
        { label: "도장공사 (발코니)", amount: 788000 },
        { label: "입주청소", amount: 396000 },
        { label: "경비 (내역 없이 총액만 표기)", amount: 446184 },
        { label: "일반관리비 (4%)", amount: 1313083 },
      ],
      notes: [
        "조명·전기 금액은 해당 페이지 소계에서 타일 합계를 뺀 값이다 (한 페이지에 두 공종이 같이 있어 소계가 합쳐져 있음).",
        "경비 446,184원은 어느 페이지에도 내역이 없다 — 무엇에 쓰는 돈인지 확인 필요.",
        "빈 공종이 있어 최종 금액이 아닐 수 있다: 안방욕실 전체, 설비(수도 배관 교체·거실/작은방 확장 난방), 창호(KCC), 시스템에어컨, 붙박이장(안방)·서랍장·선반.",
        "특기사항에 '견적내역외 물량은 별도'와 '붙박이장 옵션(서랍·거울) 선택 시 추가금 발생 가능'이 있는데, 표지에는 '공사 중 추가금 발생하지 않음'이라고 적혀 있어 서로 모순 — 계약서에 추가금 없음을 명시해달라고 요청할 것.",
        "명시된 자재는 브랜드 수준이다 (벽지 KCC신한리빙/LX베스트, 바닥재 LX 3.2T, 스위치 진흥J, 위생도기·수전 아메스/대림바스 등) — 제품명·모델번호·규격은 아직 없음.",
        "바닥재 21평은 전용 약 59㎡(약 18평)에 비해 커 보인다 — 면적 산정 기준 확인.",
        "공사명에 '24py'로 적혀 있음 (앱에는 25평) — 오기인지 확인.",
        "결제 조건(선금/기성금/잔금), 공사 기간, 계약금 반환 조건은 견적서에 없다.",
      ],
    },
  },
  {
    ...
    quote: {
      ...
      breakdown: [],
      notes: [
        "서면 견적서 없이 말로만 받은 금액이다 — 항목별 내역이 없어 공사 범위를 알 수 없다.",
        "사용하는 제품·자재를 안내받지 못했다 — 디자인큐원 견적과 같은 조건의 비교가 아니다.",
        "비교하려면 서면 견적서(항목·수량·단가·자재 브랜드/모델)를 요청해야 한다.",
      ],
    },
  },
  {
